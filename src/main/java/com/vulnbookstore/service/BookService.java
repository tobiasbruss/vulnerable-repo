...
public class BookService {
    ...
    // Allowed export formats — validated before passing to the export script
    ...
    public String exportBookData(String format) {
        if (!ALLOWED_EXPORT_FORMATS.contains(format)) {
            logger.warn("Rejected unknown export format '{}'", format);
            return "Export error: unsupported format '" + format + "'";
        }
        try {
            logger.info("Running export with format: {}", format);

            // Use array form of exec() to avoid shell injection; format is validated above
