...
public class BookService {
     ...
     *
     * Format is validated against an allowlist before use.
     * exec() is called with a String array to avoid shell interpretation.
     */
    public String exportBookData(String format) {
        if (!ALLOWED_EXPORT_FORMATS.contains(format)) {
            ...
            return "Unsupported export format. Supported formats: csv, json, xml";
        }
        try {
            logger.info("Running export with format: {}", format);
            // Use array form of exec() so the argument is never interpreted by a shell
