...
public class BookController {
    ...
    public ResponseEntity<String> exportBooks(@RequestParam("format") String format) {
        if (!ALLOWED_EXPORT_FORMATS.contains(format.toLowerCase())) {
            return ResponseEntity.badRequest().body("Invalid format. Allowed formats: csv, json, xml");
        }
        String result = bookService.exportBookData(format.toLowerCase());
