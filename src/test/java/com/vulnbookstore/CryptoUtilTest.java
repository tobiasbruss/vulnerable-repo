 ...
 *
 * The implementation uses SHA-256 for hashing/checksums and AES-256-GCM for
...
class CryptoUtilTest {
    ...
    // ----------------------------------------------------------------
    // encrypt() and decrypt() — AES-256-GCM
    // ----------------------------------------------------------------
    ...
    @Test
    @DisplayName("encrypt() uses a random IV — same input produces different ciphertext each time")
    void encrypt_usesRandomIV_differentCiphertextEachCall() {
        // AES-GCM uses a fresh random IV per call, so identical plaintexts
        // must produce different ciphertexts (unlike the old ECB mode).
        String encrypted1 = CryptoUtil.encrypt("same data");
        String encrypted2 = CryptoUtil.encrypt("same data");

        assertNotEquals(encrypted1, encrypted2,
                "AES-GCM should produce a different ciphertext for each call due to random IV");
    }
