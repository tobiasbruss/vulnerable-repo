...
class CryptoUtilTest {
    ...
    @Test
    @DisplayName("encrypt() produces different ciphertext for same input due to random IV (AES/GCM)")
    void encrypt_isNonDeterministic_dueToRandomIv() {
        // AES/GCM uses a random IV per encryption call, so the same plaintext
        // produces a different ciphertext each time — a security strength.
