...
class CryptoUtilTest {
    ...
    @Test
    @DisplayName("encrypt() produces different ciphertext on each call (GCM random IV)")
    void encrypt_isNonDeterministic_dueToGcmIv() {
        String encrypted1 = CryptoUtil.encrypt("same data");
        String encrypted2 = CryptoUtil.encrypt("same data");

        // AES/GCM uses a fresh random IV each time, so ciphertexts must differ
